...
def upgrade() -> None:
    """Execute auth RLS fix migration from SQL file"""
    sql_file = project_root / 'db' / 'migration' / '0.0.50__auth_rls_fix.sql'
...
def downgrade() -> None:
    """Downgrade: Remove the authentication RLS policy"""
    # Get the raw psycopg2 connection
    connection = op.get_bind().connection
    raw_connection = connection.connection
    
    with raw_connection.cursor() as cursor:
        cursor.execute("DROP POLICY IF EXISTS system_admin_accounts_select_for_auth ON tutor.system_admin_accounts;")
        raw_connection.commit()
